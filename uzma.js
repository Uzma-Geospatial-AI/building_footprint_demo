...
const SEREMBAN_AREAS = [
  { id:'seremban-main', name:'Seremban Town', desc:'Central business district and main urban core', icon:'🏙️', tags:['Urban','Commercial','CBD'], center:[101.9424,2.7297], zoom:14, bounds:null },
  { id:'rasah-jaya', name:'Rasah Jaya', desc:'Residential township, south-west of Seremban', icon:'🏘️', tags:['Residential','Township'], center:[101.9207,2.7015], zoom:14, bounds:null },
  { id:'nilai', name:'Nilai', desc:'Industrial and commercial hub, Seremban district', icon:'🏭', tags:['Industrial','Commercial'], center:[101.7986,2.8193], zoom:13, bounds:null },
  { id:'bahau', name:'Bahau', desc:'Agricultural and rural area, Jempol district', icon:'🌾', tags:['Agriculture','Rural'], center:[102.4263,2.8013], zoom:13, bounds:null },
  { id:'tampin', name:'Tampin', desc:'Southern Negeri Sembilan, near Melaka border', icon:'🌳', tags:['Rural','Border'], center:[102.2298,2.4699], zoom:13, bounds:null },
  { id:'port-dickson', name:'Port Dickson', desc:'Coastal resort town with beaches and tourism', icon:'🏖️', tags:['Coastal','Tourism'], center:[101.7974,2.5222], zoom:13, bounds:null },
  { id:'mantin', name:'Mantin', desc:'Small town between Seremban and Nilai', icon:'🏡', tags:['Residential','Town'], center:[101.8777,2.7896], zoom:14, bounds:null },
  { id:'senawang', name:'Senawang', desc:'Industrial estate and mixed development area', icon:'⚙️', tags:['Industrial','Mixed'], center:[101.9511,2.6799], zoom:14, bounds:null },
  { id:'paroi', name:'Paroi', desc:'Residential and light industrial zone', icon:'🏗️', tags:['Residential','Industrial'], center:[101.9677,2.7539], zoom:14, bounds:null },
  { id:'full-extent', name:'Full Seremban Extent', desc:'Fly to full loaded GeoJSON / UZMA-sat coverage area', icon:'🗺️', tags:['Dataset','Overview'], center:[101.9317,2.7287], zoom:11, bounds:[[101.87766085815322,2.6743293354532471],[101.98574632250876,2.7830234586744691]], isDataset:true },
];
...
function openAreaModal() {
  ...
  setTimeout(() => {
    ...
    _allAreaCards = dsCards.length ? [...dsCards, ...SEREMBAN_AREAS] : [...SEREMBAN_AREAS];
...
function buildDatasetAreaCards() {
  ...
  for (const [dsId, ds] of Object.entries(datasets)) {
    ...
    cards.push({ id:'ds-'+dsId, name:shortName, desc:ds.features.length.toLocaleString()+' features loaded · '+formatFileSize(ds.fileSize), icon:'📂', tags:['Dataset','GeoJSON'], center:null, zoom:null, bounds:ds.bounds, dsId, isDataset:true });
  }
  return cards;
}
...
function renderAreaCards(areas) {
  ...
  if (!areas.length) { grid.innerHTML = '<div class="area-card-empty">No areas found.</div>'; return; }
  ...
  grid.innerHTML = areas.map(a => {
    ...
    return `<button class="area-card" onclick="flyToArea('${a.id}')"><div class="area-card-icon">${a.icon}</div><div class="area-card-name">${a.name}</div><div class="area-card-desc">${a.desc}</div><div class="area-card-meta">${tagsHtml}</div></button>`;
  }).join('');
}
...
function filterAreaCards(val) {
  ...
  renderAreaCards(_allAreaCards.filter(a => a.name.toLowerCase().includes(q) || a.desc.toLowerCase().includes(q) || a.tags.some(t => t.toLowerCase().includes(q))));
}
...
function flyToArea(areaId) {
  ...
  if (!area) return;
  closeAreaModal();
  if (area.bounds) map.fitBounds(area.bounds, { padding:50, duration:1200 });
  else if (area.dsId && datasets[area.dsId]?.bounds) { map.fitBounds(datasets[area.dsId].bounds, { padding:50, duration:1200 }); setActiveDataset(area.dsId); updateDatasetBadge(); }
  else if (area.center) map.flyTo({ center:area.center, zoom:area.zoom||13, duration:1200 });
  showToast('📍 Flying to ' + area.name);
  addActivityLog('Area selected: ' + area.name, area.desc);
}

function closeAreaModal() { document.getElementById('area-overlay').classList.remove('show'); }

// ============================================================
// UZMA-SAT
// ============================================================
const UZMASAT_COORDS = {
  coordinates: [[101.87766085815322,2.7830234586744691],[101.98574632250876,2.7830234586744691],[101.98574632250876,2.6743293354532471],[101.87766085815322,2.6743293354532471]],
...
function detectProp(props, candidates) {
  for (const c of candidates) { if (props[c] !== undefined && props[c] !== null && props[c] !== '') return c; }
  return null;
}
...
function buildMapStyle(key) {
  ...
  if (bm && bm.pmtiles) return { version:8, sources:{ basemap:{ type:'raster', url:bm.url, tileSize:bm.tileSize } }, layers:[{ id:'basemap', type:'raster', source:'basemap' }] };
...
window.addEventListener('load', () => {
  if (window.pmtiles) {
    ...
    } catch(e) { console.error('❌ PMTiles registration failed:', e); }
  } else {
    console.warn('⚠️ pmtiles.js not loaded');
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (!e.target.closest('.basemap-switcher')) document.getElementById('basemap-dropdown').classList.remove('open');
    if (!e.target.closest('.area-overlay') && e.target.classList.contains('area-overlay')) closeAreaModal();
  });
});
...
function toggleTrafficLights(el) {
  // Guard check
  if (typeof trafficLightsData === 'undefined' || !trafficLightsData.features.length) {
    showToast('⚠️ Traffic lights data not available');
  ...
  if (!isOn) {
    ...
    renderTrafficLights();
    showToast('🚦 Traffic lights shown — ' + trafficLightsData.features.length + ' points');
    addActivityLog('Traffic lights active', trafficLightsData.features.length + ' points shown');
  } else {
    el.className = 'layer-toggle off';
    document.getElementById('tl-detail-panel').style.display = 'none';
    trafficLightsVisible = false;
    clearTrafficLights();
    showToast('🔲 Traffic lights hidden');
  }
}

function renderTrafficLights() {
  clearTrafficLights();
  if (typeof trafficLightsData === 'undefined') return;
  const features = trafficLightsData.features;
  const currentZoom = map.getZoom();
  if (currentZoom < 9) { showToast('⚠️ Zoom in to see traffic lights (zoom ≥ 9)'); return; }
  features.forEach(f => {
    ...
    el.textContent = '🚦';
    el.title = 'Traffic Light #' + f.properties.id;
    ...
    el.addEventListener('click', (e) => {
      ...
      new maplibregl.Popup({ offset:8, closeButton:true })
        .setLngLat([lng, lat])
        .setHTML(`<div class="popup-header" style="background:#FDD835;color:#333;"><div class="popup-fc" style="color:#666;">Traffic Light</div><div class="popup-name" style="color:#333;">🚦 ID #${f.properties.id}</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Type</span><span class="popup-v">Traffic Signal</span></div><div class="popup-row"><span class="popup-k">Latitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lat.toFixed(6)}</span></div><div class="popup-row"><span class="popup-k">Longitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lng.toFixed(6)}</span></div><div class="popup-row"><span class="popup-k">Source</span><span class="popup-v">OSM (.shp)</span></div></div>`)
        .addTo(map);
    });
    const marker = new maplibregl.Marker({ element:el, anchor:'center' }).setLngLat([lng, lat]).addTo(map);
    trafficLightMarkers.push(marker);
  });
  showToast('🚦 ' + features.length + ' traffic lights shown');
}

function clearTrafficLights() { trafficLightMarkers.forEach(m => m.remove()); trafficLightMarkers = []; }

function updateTLSize(val) {
  tlEmojiSize = parseInt(val);
  document.getElementById('tl-size-val').textContent = val + 'px';
  if (trafficLightsVisible) trafficLightMarkers.forEach(m => { if (m.getElement()) m.getElement().style.fontSize = val + 'px'; });
}
...
function toggleWaterLayer(el) {
  if (typeof waterData === 'undefined') {
    showToast('⚠️ Water data not loaded');
    el.className = 'layer-toggle off';
    return;
  }
  const isOn = el.classList.contains('on');
  if (!isOn) {
    ...
    renderWaterLayer();
    showToast('💧 Water layer shown — ' + waterData.features.length + ' areas');
    addActivityLog('Water layer active', waterData.features.length + ' water areas shown');
  } else {
    el.className = 'layer-toggle off';
    waterLayerVisible = false;
    clearWaterLayer();
    showToast('🔲 Water layer hidden');
  }
}

function renderWaterLayer() {
  clearWaterLayer();
  if (typeof waterData === 'undefined') return;
  try {
    if (map.getSource('water-source')) return;
    map.addSource('water-source', { type:'geojson', data:waterData });
    map.addLayer({ id:'water-fill', type:'fill', source:'water-source', paint: { 'fill-color':['case',['==',['get','f'],'riverbank'],'#2196F3',['==',['get','f'],'water'],'#42A5F5',['==',['get','f'],'wetland'],'#00BCD4',['==',['get','f'],'wetland_mangrove'],'#26C6DA',['==',['get','f'],'marsh'],'#80DEEA','#64B5F6'], 'fill-opacity':0.55 } });
    map.addLayer({ id:'water-outline', type:'line', source:'water-source', paint:{ 'line-color':'#1565C0','line-width':0.8,'line-opacity':0.6 } });
    map.on('click','water-fill',(e) => {
      const props = e.features[0].properties;
      new maplibregl.Popup({ offset:8 }).setLngLat(e.lngLat).setHTML(`<div class="popup-header" style="background:#1565C0;"><div class="popup-fc" style="color:rgba(255,255,255,0.6);">Hydrography</div><div class="popup-name">💧 ${props.n||'Water Body'}</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Class</span><span class="popup-v">${props.f||'water'}</span></div><div class="popup-row"><span class="popup-k">Source</span><span class="popup-v">OSM</span></div></div>`).addTo(map);
    });
...
// RAILWAY LAYER
// ============================================================
const RAILWAY_COLORS = { rail:'#FF5722', monorail:'#9C27B0', subway:'#3F51B5', light_rail:'#00BCD4', tram:'#4CAF50' };
const RAILWAY_EMOJIS = { rail:'🚆', monorail:'🚝', subway:'🚇', light_rail:'🚊', tram:'🚃' };

function toggleRailwayLayer(el) {
  if (typeof railwayData === 'undefined') {
    showToast('⚠️ Railway data not loaded');
    el.className = 'layer-toggle off';
    return;
  }
  const isOn = el.classList.contains('on');
  if (!isOn) {
    ...
    renderRailwayLayer();
    showToast('🚆 Railway lines shown — ' + railwayData.features.length + ' segments');
    addActivityLog('Railway layer active', railwayData.features.length + ' segments shown');
  } else {
    el.className = 'layer-toggle off';
    railwayLayerVisible = false;
    clearRailwayLayer();
    showToast('🔲 Railway lines hidden');
  }
}

function renderRailwayLayer() {
  clearRailwayLayer();
  if (typeof railwayData === 'undefined') return;
  try {
    if (map.getSource('railway-source')) return;
    map.addSource('railway-source', { type:'geojson', data:railwayData });
    map.addLayer({ id:'railway-glow', type:'line', source:'railway-source', paint:{ 'line-color':['case',['==',['get','f'],'rail'],'#FF5722',['==',['get','f'],'monorail'],'#9C27B0',['==',['get','f'],'subway'],'#3F51B5',['==',['get','f'],'light_rail'],'#00BCD4','#FF5722'], 'line-width':6,'line-opacity':0.15,'line-blur':4 } });
    map.addLayer({ id:'railway-line', type:'line', source:'railway-source', paint:{ 'line-color':['case',['==',['get','f'],'rail'],'#FF5722',['==',['get','f'],'monorail'],'#9C27B0',['==',['get','f'],'subway'],'#3F51B5',['==',['get','f'],'light_rail'],'#00BCD4','#FF5722'], 'line-width':['interpolate',['linear'],['zoom'],8,1.5,14,4],'line-opacity':0.9 } });
    map.addLayer({ id:'railway-dash', type:'line', source:'railway-source', filter:['==',['get','f'],'rail'], paint:{ 'line-color':'#fff','line-width':['interpolate',['linear'],['zoom'],8,0.5,14,1.5],'line-dasharray':[4,8],'line-opacity':0.6 } });
    map.on('click','railway-line',(e) => {
      ...
      new maplibregl.Popup({ offset:8 }).setLngLat(e.lngLat).setHTML(`<div class="popup-header" style="background:${color};"><div class="popup-fc" style="color:rgba(255,255,255,0.6);">Railway</div><div class="popup-name">${emoji} ${props.n||'Railway Line'}</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Type</span><span class="popup-v">${fclass}</span></div><div class="popup-row"><span class="popup-k">Source</span><span class="popup-v">OSM</span></div></div>`).addTo(map);
    });
...
function toggleRoadNetwork(el) {
  ...
  if (!isOn) {
    ...
    showToast('🛣️ Road network — using OSM basemap');
...
function loadUzmaSatLayer() {
  removeUzmaSatLayer();
  try {
    map.addSource('uzmasat-source', { type:'raster', url:PMTILES_FULL_URL, tileSize:256, attribution:'© UZMA Berhad' });
    map.addLayer({ id:'uzmasat-layer', type:'raster', source:'uzmasat-source', paint:{ 'raster-opacity':uzmaSatOpacity,'raster-fade-duration':300 } }, getFirstGeoJsonLayerId());
    console.log('✅ UZMA-sat loaded');
  } catch(e) {
    console.error('UZMA-sat load error:', e);
    showToast('⚠️ PMTiles could not be loaded — check seremban.pmtiles in repo root');
  }
}
...
function activateUzmaSat(el) {
  ...
  if (!isOn) {
    ...
    showToast('🛰️ UZMA-sat loaded — seremban.pmtiles');
  } else {
    el.className = 'layer-toggle off';
...
function addDataset(fileName, geojson, fileSize, isDefault=false) {
  ...
  const classCount = {};
  features.forEach(f => { const cls = classProp ? (f.properties[classProp]||'Other') : 'Data'; classCount[cls] = (classCount[cls]||0) + 1; });

  const mukimCount = {};
  if (mukimProp) features.forEach(f => { const m = f.properties[mukimProp]||'Unknown'; mukimCount[m] = (mukimCount[m]||0) + 1; });
  ...
  datasets[dsId] = { id:dsId, name:fileName, geojson:processedGeoJSON, features, classCount, mukimCount, classProp, idProp, mukimProp, addrProp, layerVisibility:{}, bounds:hasCoords&&isFinite(minLng)?[[minLng,minLat],[maxLng,maxLat]]:null, fileSize, loadedAt:new Date() };
...
function addLayersForDataset(dsId) {
  ...
  classes.forEach(cls => {
    ...
    map.addLayer({ id:fillId, type:'fill', source:sourceId, filter, paint:{ 'fill-color':color, 'fill-opacity':['case',['boolean',['feature-state','hover'],false],0.75,['boolean',['feature-state','selected'],false],0.85,0.45] } });
    map.addLayer({ id:outlineId, type:'line', source:sourceId, filter, paint:{ 'line-color':['case',['boolean',['feature-state','hover'],false],'#FFFFFF',color], 'line-width':['interpolate',['linear'],['zoom'],12,['case',['boolean',['feature-state','hover'],false],2,0.5],16,['case',['boolean',['feature-state','hover'],false],3,1.5]], 'line-opacity':['case',['boolean',['feature-state','hover'],false],1,0.7] } });
...
function countClass(classCount, names) {
  let t = 0;
  for (const [k,v] of Object.entries(classCount)) { if (names.some(n => k.toLowerCase().includes(n.toLowerCase()))) t += v; }
  return t;
}
...
function updateLayerBadge(dsId, visibility) {
  ...
  for (const [cls, info] of Object.entries(visibility)) {
    ...
    div.innerHTML = `<div class="layer-dot" style="background:${info.color};"></div><div class="layer-name">${cls}</div><div class="layer-count">${info.count.toLocaleString()}</div><div class="layer-toggle on" id="${safeId}"></div>`;
...
function updateDatasetModalList() {
  ...
  for (const [dsId, ds] of Object.entries(datasets)) {
    ...
    div.innerHTML = `<div><div class="dm-name">${shortName}</div><div class="dm-count">${ds.features.length.toLocaleString()} features · ${formatFileSize(ds.fileSize)}</div></div><div style="display:flex;align-items:center;gap:6px;">${isActive?'<span class="dm-active">Active</span>':''}<button class="dm-btn">Remove</button></div>`;
...
function updateOverviewCharts(classCount, mukimCount, total, fileName, fileSize, loadedAt) {
  ...
  if (heroVal) { heroVal.textContent = total.toLocaleString(); const heroSub = document.getElementById('summary-hero-sub'); if (heroSub) heroSub.textContent = fileName.replace(/\.(geojsonn?|json)$/i,'')+' · Negeri Sembilan'; }
  ...
  if (activeDatasetId && datasets[activeDatasetId]) {
    ...
    if (pcDiv && Object.keys(postcodeCount).length) {
      ...
      pcDiv.innerHTML = pcSorted.map(([pc,cnt],i) => { const w=Math.max(3,(cnt/pcMax)*100); const pct=((cnt/total)*100).toFixed(1); return `<div class="postcode-row"><span class="postcode-label">${pc||'?'}</span><div class="postcode-bar-track"><div class="postcode-bar-fill" style="width:${w}%;background:${pcColors[i%pcColors.length]};"></div></div><span class="postcode-count">${cnt.toLocaleString()} (${pct}%)</span></div>`; }).join('');
    } else if (pcDiv) { pcDiv.innerHTML = '<div style="font-size:11px;color:rgba(66,66,66,0.38);padding:6px 0;">No postcode data</div>'; }
  }
  ...
  } else { mukimList.innerHTML = '<div style="font-size:11.5px;color:rgba(66,66,66,0.38);text-align:center;padding:7px;">No sub-district data</div>'; }
...
function showParcelInfo(feature, lngLat) {
  ...
  const popupRows = Object.entries(props).slice(0,6).map(([k,v]) => `<div class="popup-row"><span class="popup-k">${k}</span><span class="popup-v">${v}</span></div>`).join('');
  currentPopup = new maplibregl.Popup({ offset:8 }).setLngLat(lngLat).setHTML(`<div class="popup-header"><div class="popup-fc">${cls||'Feature'}</div><div class="popup-name">${lotId}</div></div><div class="popup-body">${popupRows}</div>`).addTo(map);
  ...
  for (const [k,v] of Object.entries(props)) html += `<div class="pc-row"><span class="pc-k">${k}</span><span class="pc-v">${v??'—'}</span></div>`;
  propsContainer.innerHTML = html;
  const c = lngLat;
  propsContainer.innerHTML += `<div class="pc-section-title">📐 Click Coordinates</div><div class="pc-row"><span class="pc-k">Latitude</span><span class="pc-v" id="p-lat" style="font-family:monospace;font-size:9.5px;">${c.lat.toFixed(7)}</span></div><div class="pc-row"><span class="pc-k">Longitude</span><span class="pc-v" id="p-lng" style="font-family:monospace;font-size:9.5px;">${c.lng.toFixed(7)}</span></div>`;
  ...
  document.querySelectorAll('.ptab').forEach(t => { if (t.getAttribute('onclick')&&t.getAttribute('onclick').includes('parcel')) t.classList.add('active'); });
}
...
function handleSearch(val) {
  clearTimeout(_searchTimeout); clearTimeout(_geocodeTimeout);
  const dd = document.getElementById('search-dropdown');
  if (!val.trim()) { dd.style.display = 'none'; return; }
  const localResults = getLocalSearchResults(val);
  if (localResults.length > 0) { renderSearchDropdown(localResults,[]); dd.style.display = 'block'; }
  else { dd.innerHTML = `<div class="search-loading"><div class="search-spinner"></div> Searching for address...</div>`; dd.style.display = 'block'; }
  if (val.trim().length >= 3) _geocodeTimeout = setTimeout(() => runGeocode(val, localResults), 600);
}
...
function getLocalSearchResults(val) {
  ...
  const q = val.toLowerCase();
  return ds.features.filter(f => Object.values(f.properties||{}).some(v => String(v).toLowerCase().includes(q))).slice(0,8);
}
...
async function runGeocode(query, localResults) {
  ...
  try {
    ...
    if (!localResults.length && !geocodeResults.length) dd.innerHTML = `<div style="padding:12px 13px;font-size:12px;color:rgba(66,66,66,0.4);">No results found</div>`;
  } catch(e) {
    renderSearchDropdown(localResults,[]);
    if (!localResults.length) dd.innerHTML = `<div style="padding:12px 13px;font-size:12px;color:rgba(66,66,66,0.4);">No results found</div>`;
  }
...
function renderSearchDropdown(localResults, geocodeResults) {
  ...
  dd.innerHTML = html;
  dd.querySelectorAll('.local-result').forEach(el => { el.addEventListener('click', () => selectSearchResult(parseInt(el.getAttribute('data-idx')))); });
  dd.querySelectorAll('.geocode-result').forEach(el => { el.addEventListener('click', () => flyToGeocode(parseFloat(el.getAttribute('data-lat')),parseFloat(el.getAttribute('data-lng')),el.getAttribute('data-name'))); });
}
...
function flyToGeocode(lat, lng, name) {
  ...
  setTimeout(() => {
    new maplibregl.Popup({ offset:8, closeOnClick:true }).setLngLat([lng,lat]).setHTML(`<div class="popup-header" style="background:#1E88E5;"><div class="popup-fc">Address Found</div><div class="popup-name" style="font-size:11px;line-height:1.4;">${name}</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Latitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lat.toFixed(6)}</span></div><div class="popup-row"><span class="popup-k">Longitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lng.toFixed(6)}</span></div></div>`).addTo(map);
  }, 800);
...
function toggleGeoCat(cat, el) {
  ...
  const newState = !isOn;
  if (!activeDatasetId) { showToast(newState?'⚠️ No active data for this category':''); if (newState) el.className='layer-toggle off'; return; }
  const ds = datasets[activeDatasetId];
  const affectedClasses = GEO_CAT_CLASS_MAP[cat];
  if (!affectedClasses) { if (newState) { showToast(`ℹ️ ${getCatName(cat)}: Data not yet integrated`); el.className='layer-toggle off'; GEO_CAT_STATE[cat]=false; } return; }
  let toggled = 0;
...
function addPlotMarker(lngLat) {
  ...
  markerEl.addEventListener('click', e => {
    e.stopPropagation();
    new maplibregl.Popup({ offset:10 }).setLngLat(lngLat).setHTML(`<div class="popup-header" style="background:${markerColor};padding:9px 14px;"><div class="marker-popup-title">Marker #${markerData.idx}</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Latitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lngLat.lat.toFixed(7)}</span></div><div class="popup-row"><span class="popup-k">Longitude</span><span class="popup-v" style="font-family:monospace;font-size:10px;">${lngLat.lng.toFixed(7)}</span></div><div class="popup-row"><span class="popup-k">Shape</span><span class="popup-v">${markerData.shape}</span></div><div style="margin-top:9px;"><button id="del-marker-${markerData.idx}" style="width:100%;height:28px;border:1px solid rgba(229,57,53,0.3);background:none;border-radius:6px;font-family:'DM Sans',sans-serif;font-size:11px;color:#e53935;cursor:pointer;">🗑️ Remove</button></div></div>`).addTo(map);
...
function removeMarkerByIdx(idx) {
  const i = plotMarkers.findIndex(m => m.idx===idx); if (i===-1) return;
  plotMarkers[i].marker.remove(); plotMarkers.splice(i,1); updateMarkerCount(); showToast('🗑️ Marker removed');
}
...
// ============================================================
const POI_EMOJIS = {
  school:'🏫',hospital:'🏥',clinic:'🏥',park:'🌳',playground:'🛝',mall:'🛍️',supermarket:'🛒',marketplace:'🏪',restaurant:'🍽️',fast_food:'🍔',cafe:'☕',food_court:'🍱',hotel:'🏨',hostel:'🏨',chalet:'🏡',university:'🎓',college:'🎓',kindergarten:'👶',police:'👮',fire_station:'🚒',library:'📚',community_centre:'🏛️',sports_centre:'⚽',swimming_pool:'🏊',golf_course:'⛳',stadium:'🏟️',pitch:'⚽',track:'🏃',attraction:'🎡',water_tower:'🗼',water_works:'💧',wastewater_plant:'🏭',graveyard:'⚰️',shelter:'⛺',toilet:'🚻',fountain:'⛲',post_office:'📮',car_dealership:'🚗',furniture_shop:'🪑',clothes:'👗',department_store:'🏬',convenience:'🏪',bank:'🏦',pharmacy:'💊',dentist:'🦷',veterinary:'🐾',cinema:'🎬',theatre:'🎭',museum:'🏛️',art:'🎨',mosque:'🕌',church:'⛪',temple:'⛩️',airport:'✈️',helipad:'🚁',bus_station:'🚌',ferry_terminal:'⛴️',railway_station:'🚆',taxi:'🚕',charging_station:'⚡',parking:'🅿️',bicycle:'🚲',
};
const POI_COLORS = {
  education:{classes:['school','university','college','kindergarten','library'],color:'#4CAF50',label:'Education'},
  health:{classes:['hospital','clinic','pharmacy','dentist','veterinary'],color:'#F44336',label:'Health'},
  recreation:{classes:['park','playground','sports_centre','swimming_pool','golf_course','stadium','pitch','track','attraction'],color:'#2196F3',label:'Recreation'},
  food:{classes:['restaurant','fast_food','cafe','food_court','supermarket','marketplace','convenience','mall','department_store'],color:'#FF9800',label:'Food & Shopping'},
  accommodation:{classes:['hotel','hostel','chalet'],color:'#9C27B0',label:'Accommodation'},
  services:{classes:['police','fire_station','community_centre','post_office','toilet','shelter','water_tower','water_works','wastewater_plant','graveyard','fountain'],color:'#607D8B',label:'Services'},
  other:{classes:[],color:'#9E9E9E',label:'Others'},
};

function getPOIColor(fclass) {
  for (const [,cat] of Object.entries(POI_COLORS)) { if (cat.classes.includes(fclass)) return cat.color; }
  return '#9E9E9E';
}

function togglePoisLayer(el) {
  if (typeof POIS_DATA === 'undefined') {
    showToast('⚠️ POI data not loaded');
    el.className = 'layer-toggle off';
    return;
  }
  const isOn = el.classList.contains('on');
  if (!isOn) {
    ...
    showToast('🏛️ POI shown — '+total.toLocaleString()+' locations');
    addActivityLog('POI layer active', total.toLocaleString()+' locations (128 classes)');
  } else {
    el.className = 'layer-toggle off';
...
function renderPoisFilterGrid() {
  ...
  const catBtn = (key, cat) => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!isActive) poisLayerFilter[key] = true; else delete poisLayerFilter[key];
      renderPoisLayer(Object.keys(poisLayerFilter).length ? poisLayerFilter : null);
    });
    grid.appendChild(btn);
  };
  for (const [key,cat] of Object.entries(POI_COLORS)) { if (key!=='other') catBtn(key,cat); }
  ...
  allBtn.addEventListener('click', () => { grid.querySelectorAll('button[data-key]').forEach(b => { b.dataset.active='false'; const c=POI_COLORS[b.dataset.key]; b.style.background=c.color+'10'; b.style.color=c.color; }); poisLayerFilter={}; renderPoisLayer(null); });
  grid.insertBefore(allBtn, grid.firstChild);
}

function renderPoisLayer(filterCats) {
  clearPoisMarkers();
  if (!poisLayerVisible || typeof POIS_DATA === 'undefined') return;
  const zoom = map.getZoom();
  const maxPerClass = zoom<10?20:zoom<12?100:zoom<14?500:9999;
  for (const [fclass,pts] of Object.entries(POIS_DATA)) {
    if (filterCats) { let matched=false; for (const [catKey,cat] of Object.entries(POI_COLORS)) { if (filterCats[catKey]&&cat.classes.includes(fclass)) { matched=true; break; } } if (!matched) continue; }
    const emoji = POI_EMOJIS[fclass]||'📍';
    const color = getPOIColor(fclass);
    pts.slice(0,maxPerClass).forEach(([lng,lat,name]) => {
      const el = document.createElement('div');
      ...
      el.addEventListener('click', e => {
        e.stopPropagation();
        new maplibregl.Popup({ offset:8 }).setLngLat([lng,lat]).setHTML('<div class="popup-header" style="background:'+color+';"><div class="popup-fc" style="color:rgba(255,255,255,0.65);">POI · '+fclass+'</div><div class="popup-name">'+emoji+' '+(name||fclass)+'</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Type</span><span class="popup-v">'+fclass+'</span></div><div class="popup-row"><span class="popup-k">Lat</span><span class="popup-v" style="font-family:monospace;font-size:10px;">'+lat.toFixed(6)+'</span></div><div class="popup-row"><span class="popup-k">Lng</span><span class="popup-v" style="font-family:monospace;font-size:10px;">'+lng.toFixed(6)+'</span></div><div class="popup-row"><span class="popup-k">Source</span><span class="popup-v">OSM</span></div></div>').addTo(map);
      });
      const marker = new maplibregl.Marker({ element:el, anchor:'center' }).setLngLat([lng,lat]).addTo(map);
      poisMarkers.push(marker);
    });
  }
  document.getElementById('pois-count-badge').textContent = poisMarkers.length.toLocaleString()+' pts';
}

function clearPoisMarkers() { poisMarkers.forEach(m => m.remove()); poisMarkers=[]; }

// ============================================================
// TRANSPORT LAYER
// ============================================================
const TRANSPORT_EMOJIS = { airport:'✈️',airfield:'🛩️',apron:'🛫',helipad:'🚁',bus_station:'🚌',bus_stop:'🚏',ferry_terminal:'⛴️',railway_station:'🚉',railway_halt:'🚂',taxi:'🚕' };
const TRANSPORT_COLORS = { airport:'#FF5722',airfield:'#FF7043',apron:'#FFAB91',helipad:'#9C27B0',bus_station:'#1E88E5',bus_stop:'#42A5F5',ferry_terminal:'#00BCD4',railway_station:'#F44336',railway_halt:'#E57373',taxi:'#FDD835' };
const TRANSPORT_LABELS = { airport:'Airport',airfield:'Airfield',apron:'Apron',helipad:'Helipad',bus_station:'Bus Station',bus_stop:'Bus Stop',ferry_terminal:'Ferry Terminal',railway_station:'KTM/LRT Station',railway_halt:'KTM Halt',taxi:'Taxi' };

function toggleTransportLayer(el) {
  if (typeof TRANSPORT_DATA === 'undefined') {
    showToast('⚠️ Transport data not loaded');
    el.className = 'layer-toggle off';
    return;
  }
  const isOn = el.classList.contains('on');
  if (!isOn) {
    ...
    showToast('🚌 Transport shown — '+total.toLocaleString()+' locations');
    addActivityLog('Transport layer active', total.toLocaleString()+' locations (10 classes)');
  } else {
    el.className = 'layer-toggle off';
    document.getElementById('transport-detail-panel').style.display = 'none';
    transportLayerVisible = false;
    clearTransportMarkers();
    showToast('🔲 Transport hidden');
  }
}

function renderTransportTypeList() {
  if (typeof TRANSPORT_DATA === 'undefined') return;
  const list = document.getElementById('transport-type-list');
  list.innerHTML = '';
  for (const [fclass,pts] of Object.entries(TRANSPORT_DATA)) {
    const emoji=TRANSPORT_EMOJIS[fclass]||'📍', color=TRANSPORT_COLORS[fclass]||'#607D8B', label=TRANSPORT_LABELS[fclass]||fclass;
    const btn = document.createElement('button');
    ...
    btn.dataset.fclass = fclass; btn.dataset.active = 'true';
    btn.addEventListener('click', () => { const isActive=btn.dataset.active==='true'; btn.dataset.active=isActive?'false':'true'; btn.style.background=isActive?color+'10':color; btn.style.color=isActive?color:'white'; renderTransportLayer(getActiveTransportFilters()); });
...
function renderTransportLayer(filterClasses) {
  clearTransportMarkers();
  if (!transportLayerVisible||typeof TRANSPORT_DATA==='undefined') return;
  for (const [fclass,pts] of Object.entries(TRANSPORT_DATA)) {
    ...
    pts.forEach(([lng,lat,name]) => {
      ...
      el.addEventListener('click', e => {
        e.stopPropagation();
        new maplibregl.Popup({ offset:8 }).setLngLat([lng,lat]).setHTML('<div class="popup-header" style="background:'+color+';"><div class="popup-fc" style="color:rgba(255,255,255,0.65);">Transport</div><div class="popup-name">'+emoji+' '+(name||label)+'</div></div><div class="popup-body"><div class="popup-row"><span class="popup-k">Type</span><span class="popup-v">'+label+'</span></div><div class="popup-row"><span class="popup-k">Class</span><span class="popup-v">'+fclass+'</span></div><div class="popup-row"><span class="popup-k">Lat</span><span class="popup-v" style="font-family:monospace;font-size:10px;">'+lat.toFixed(6)+'</span></div><div class="popup-row"><span class="popup-k">Lng</span><span class="popup-v" style="font-family:monospace;font-size:10px;">'+lng.toFixed(6)+'</span></div><div class="popup-row"><span class="popup-k">Source</span><span class="popup-v">OSM</span></div></div>').addTo(map);
      });
      const marker = new maplibregl.Marker({ element:el, anchor:'center' }).setLngLat([lng,lat]).addTo(map);
...
function clearTransportMarkers() { transportMarkers.forEach(m => m.remove()); transportMarkers=[]; }
...
function setMapMode(mode, el) {
  mapMode = mode;
  const mapEl=document.getElementById('map'), cursor=document.getElementById('custom-cursor'), indicator=document.getElementById('mode-indicator'), toolbar=document.getElementById('plot-toolbar');
  document.querySelectorAll('.map-ctrl-btn').forEach(b => { if (b.id?.startsWith('mode-')) b.classList.remove('active','select-active','plot-active'); });
  indicator.classList.remove('show','plot-mode'); toolbar.classList.remove('show');
  mapEl.classList.remove('cursor-select-mode','cursor-plot-mode'); cursor.classList.remove('visible');
  if (mode==='select') { el.classList.add('select-active'); mapEl.classList.add('cursor-select-mode'); cursor.classList.add('visible'); indicator.classList.add('show'); document.getElementById('mode-indicator-text').textContent='Lot Select Mode — Click a lot for information'; }
  else if (mode==='plot') { el.classList.add('plot-active'); mapEl.classList.add('cursor-plot-mode'); indicator.classList.add('show','plot-mode'); document.getElementById('mode-indicator-text').textContent='Plot Marker Mode — Click on the map to place a marker'; toolbar.classList.add('show'); }
  else { el.classList.add('active'); map.getCanvas().style.cursor='grab'; }
}

// ============================================================
// BASEMAP
// ============================================================
function toggleBasemapDropdown() { document.getElementById('basemap-dropdown').classList.toggle('open'); }
...
function switchBasemap(key, label, el) {
  ...
  if (key==='uzma-sat') {
    ...
    if (!uzmaSatActive) { uzmaSatActive=true; document.getElementById('tog-uzmasat').className='layer-toggle on'; document.getElementById('uzmasat-layer-info').style.display='block'; document.getElementById('badge-uzmasat').textContent='Active'; document.getElementById('badge-uzmasat').classList.add('demo'); map.fitBounds(UZMASAT_COORDS.bounds,{ padding:40, duration:1200 }); addActivityLog('UZMA-sat activated','seremban.pmtiles · PMTiles · zoom 0–16'); showToast('🛰️ UZMA-sat loaded · seremban.pmtiles · zoom 0–16'); }
  ...
  if (uzmaSatActive && key!=='uzma-sat') {
    removeUzmaSatLayer(); uzmaSatActive=false;
    document.getElementById('tog-uzmasat').className='layer-toggle off';
    document.getElementById('uzmasat-layer-info').style.display='none';
    document.getElementById('badge-uzmasat').textContent='Available';
    document.getElementById('badge-uzmasat').classList.remove('demo');
    if (!geoJsonOverlayVisible) {
      geoJsonOverlayVisible=true; document.getElementById('tog-overlay-on').className='layer-toggle on';
      for (const [,ds] of Object.entries(datasets)) { for (const [,info] of Object.entries(ds.layerVisibility)) { if (info.visible) { try { map.setLayoutProperty(info.fillId,'visibility','visible'); } catch(e) {} try { map.setLayoutProperty(info.outlineId,'visibility','visible'); } catch(e) {} } } }
    }
  }
  const bm = BASEMAPS[key];
  if (bm && !bm.pmtiles) {
    try { map.getSource('basemap')?.setTiles(bm.tiles); }
    catch(e) {
      const center=map.getCenter(), zoom=map.getZoom();
      map.setStyle(buildMapStyle(key));
      map.once('style.load', () => { map.setCenter(center); map.setZoom(zoom); for (const dsId of Object.keys(datasets)) addLayersForDataset(dsId); });
    }
  }
}
  ...
  reader.onload = e => {
    ...
    setTimeout(() => {
      try {
        ...
        setTimeout(() => { closeUploadModal(); showLoading('Loading '+file.name,'Rendering '+(geojson.features?.length||0).toLocaleString()+' features...'); setTimeout(() => { addDataset(file.name,geojson,file.size); hideLoading(); }, 300); }, 200);
...
function selectExportType(type, el) {
  ...
  document.getElementById('exp-setting-title').style.display   = type==='pdf'?'flex':'none';
  document.getElementById('exp-setting-map').style.display     = type==='pdf'?'flex':'none';
  document.getElementById('exp-setting-markers').style.display = (type==='pdf'||type==='png')?'flex':'none';
  document.getElementById('exp-setting-stats').style.display   = type==='pdf'?'flex':'none';
}
...
async function captureMapWithMarkers() {
  ...
  return await html2canvas(mapWrapper, { useCORS:true, allowTaint:true, scale:1.5, logging:false, ignoreElements:(el) => el.classList?.contains('map-controls')||el.classList?.contains('mode-indicator')||el.classList?.contains('no-data-state')||el.classList?.contains('plot-toolbar')||el.id==='selection-pulse' });
}

async function runExport() {
  const type=currentExportType, title=document.getElementById('exp-title-input').value||'Geospatial Report';
  const includeMap=document.getElementById('exp-include-map').checked, includeMarkers=document.getElementById('exp-include-markers').checked, includeStats=document.getElementById('exp-include-stats').checked;
  ...
  try {
    if (type==='geojson') {
      ...
      const ds=datasets[activeDatasetId];
      triggerDownload(new Blob([JSON.stringify(ds.geojson,null,2)],{ type:'application/json' }),ds.name.replace(/\.(geojsonn?|json)$/i,'')+'-export.geojson');
      closeExportModal(); showToast('✅ GeoJSON exported');
    } else if (type==='csv') {
      if (!activeDatasetId) { showToast('⚠️ No active dataset'); return; }
      const ds=datasets[activeDatasetId];
      const headers=new Set(); ds.features.forEach(f => Object.keys(f.properties||{}).forEach(k => headers.add(k)));
      const cols=[...headers];
  ...
  } catch(err) { console.error('Export error:',err); showToast('❌ Export failed: '+err.message); }
  finally { document.getElementById('export-progress').style.display='none'; }
}
...
async function exportPDF(title, includeMap, includeMarkers, includeStats) {
  ...
  y=34;
  if (uzmaSatActive) { pdf.setFillColor(254,248,243); pdf.setDrawColor(222,145,81); pdf.setLineWidth(0.3); pdf.rect(margin,y,W-margin*2,7,'FD'); pdf.setFont('helvetica','bold'); pdf.setFontSize(7); pdf.setTextColor(192,114,64); pdf.text('🛰 UZMA-sat ACTIVE: seremban.pmtiles · PMTiles · WGS84 · Seremban Area, Negeri Sembilan',margin+3,y+4.5); y+=10; }
  if (includeMap) {
    document.getElementById('export-progress-text').textContent='Capturing map screenshot...';
    try {
      const canvas=await captureMapWithMarkers();
      const imgData=canvas.toDataURL('image/jpeg',0.88);
      const mapW=W-margin*2, mapH=Math.min(Math.round(mapW*canvas.height/canvas.width),90);
      pdf.setDrawColor(220,220,220); pdf.setLineWidth(0.3); pdf.rect(margin,y,mapW,mapH);
      pdf.addImage(imgData,'JPEG',margin,y,mapW,mapH); y+=mapH+4;
      pdf.setFontSize(7); pdf.setTextColor(150,150,150); pdf.setFont('helvetica','normal');
      pdf.text('Figure 1: Current map screenshot'+(uzmaSatActive?' with UZMA-sat overlay':'')+' · Basemap: '+document.getElementById('basemap-label').textContent+' · '+plotMarkers.length+' marker(s)',margin,y); y+=7;
    } catch(e) { console.warn('Map capture failed:',e); pdf.setFontSize(8); pdf.setTextColor(150,150,150); pdf.text('[Map could not be exported]',margin,y+5); y+=12; }
  }
  if (includeMarkers && plotMarkers.length > 0) {
    if (y>H-50) { pdf.addPage(); y=margin; } y+=2;
    ...
    pdf.setFillColor(46,46,46); pdf.rect(margin,y,W-margin*2,6,'F'); pdf.setTextColor(255,255,255); pdf.setFontSize(7.5);
    let cx=margin+2; headers.forEach((h,i) => { pdf.text(h,cx,y+4); cx+=colW[i]; }); y+=6;
    plotMarkers.slice(0,20).forEach((m,i) => {
      pdf.setFillColor(i%2===0?250:255,i%2===0?248:255,i%2===0?243:255); pdf.rect(margin,y,W-margin*2,5.5,'F');
      pdf.setTextColor(66,66,66); pdf.setFontSize(7); cx=margin+2;
      [m.idx,m.lngLat.lat.toFixed(6),m.lngLat.lng.toFixed(6),m.shape,m.color].forEach((v,j) => { pdf.text(String(v),cx,y+3.8); cx+=colW[j]; }); y+=5.5;
    });
    if (plotMarkers.length>20) { pdf.setFontSize(7); pdf.setTextColor(150,150,150); pdf.text('... and '+(plotMarkers.length-20)+' more',margin,y+3); y+=6; } y+=4;
  }
  if (includeStats && activeDatasetId) {
    ...
    const boxes=[{ label:'Total Lots',val:ds.features.length.toLocaleString() },{ label:'Land Use Classes',val:Object.keys(ds.classCount).length },{ label:'Plot Markers',val:plotMarkers.length }];
    const bw=(W-margin*2-8)/3;
    boxes.forEach((b,i) => { const bx=margin+i*(bw+4); pdf.setFillColor(254,248,243); pdf.setDrawColor(222,145,81); pdf.setLineWidth(0.5); pdf.roundedRect(bx,y,bw,14,2,2,'FD'); pdf.setFont('helvetica','bold'); pdf.setFontSize(11); pdf.setTextColor(46,46,46); pdf.text(String(b.val),bx+bw/2,y+7,{ align:'center' }); pdf.setFont('helvetica','normal'); pdf.setFontSize(6.5); pdf.setTextColor(150,150,150); pdf.text(b.label,bx+bw/2,y+11.5,{ align:'center' }); });
    y+=18; pdf.setFont('helvetica','bold'); pdf.setFontSize(8); pdf.setTextColor(46,46,46);
    ...
    Object.entries(ds.classCount).sort((a,b) => b[1]-a[1]).slice(0,12).forEach(([cls,count],i) => {
      ...
      pdf.text(cls,margin+2,y+3.8); pdf.text(count.toLocaleString(),margin+90,y+3.8); pdf.text(((count/total)*100).toFixed(1)+'%',margin+115,y+3.8);
      const barW=(count/total)*(W-margin*2-135); pdf.setFillColor(222,145,81); pdf.rect(margin+130,y+1.5,barW,2.5,'F'); y+=5.5;
    }); y+=5;
  }
  const totalPages=pdf.internal.getNumberOfPages();
  for (let pg=1;pg<=totalPages;pg++) { pdf.setPage(pg); pdf.setFillColor(46,46,46); pdf.rect(0,H-10,W,10,'F'); pdf.setFont('helvetica','normal'); pdf.setFontSize(6.5); pdf.setTextColor(200,200,200); pdf.text('Geospatial AI — UZMA Berhad | Generated on '+new Date().toLocaleString('en-MY'),margin,H-4); pdf.text('Page '+pg+' / '+totalPages,W-margin,H-4,{ align:'right' }); }
  ...
  closeExportModal(); showToast('✅ PDF exported successfully!'); addActivityLog('PDF exported',title);
}

function triggerDownload(blob, filename) { const url=URL.createObjectURL(blob), a=document.createElement('a'); a.href=url; a.download=filename; a.click(); URL.revokeObjectURL(url); }

// ============================================================
// UTILITY
// ============================================================
function flyToData() { if (uzmaSatActive) { map.fitBounds(UZMASAT_COORDS.bounds,{ padding:40, duration:800 }); return; } if (window._dataBounds) map.fitBounds(window._dataBounds,{ padding:60, duration:800 }); }
function zoomToSelected() { if (!selectedFeature) return; const coords=flattenCoords(selectedFeature.geometry); if (!coords.length) return; const avgLng=coords.reduce((s,[lng]) => s+lng,0)/coords.length, avgLat=coords.reduce((s,[,lat]) => s+lat,0)/coords.length; map.flyTo({ center:[avgLng,avgLat], zoom:17 }); }
function copyCoords() { const lat=document.getElementById('p-lat')?.textContent, lng=document.getElementById('p-lng')?.textContent; if (lat&&lng&&lat!=='—') { navigator.clipboard.writeText(`${lat}, ${lng}`); showToast('📋 Coordinates copied'); } }
function switchTab(el, tabId) { document.querySelectorAll('.ptab').forEach(t => t.classList.remove('active')); el.classList.add('active'); document.querySelectorAll('[id^="tab-"]').forEach(t => t.style.display='none'); document.getElementById('tab-'+tabId).style.display='block'; }

// ============================================================
// LOT LIST
// ============================================================
let llAllRows=[],llFilteredRows=[],llPage=1;
const LL_PAGE_SIZE=50;
let llSortCol=null,llSortDir=1,llColumns=[];

function openLotListModal() { document.getElementById('lotlist-overlay').classList.add('show'); buildLotList(); }
function closeLotListModal() { document.getElementById('lotlist-overlay').classList.remove('show'); }

function buildLotList() {
  if (!activeDatasetId||!datasets[activeDatasetId]) {
    document.getElementById('lotlist-no-data').style.display='block'; document.getElementById('lotlist-table').style.display='none';
    document.getElementById('lotlist-pagination').style.display='none'; document.getElementById('lotlist-sub').textContent='No dataset loaded'; document.getElementById('lotlist-count-badge').textContent='0 lots'; return;
  }
  const ds=datasets[activeDatasetId], features=ds.features;
  const keyCounts={}; features.forEach(f => { Object.keys(f.properties||{}).forEach(k => { keyCounts[k]=(keyCounts[k]||0)+1; }); });
  ...
  Object.keys(ds.classCount).sort().forEach(cls => { classSel.innerHTML+=`<option value="${cls}">${cls} (${ds.classCount[cls].toLocaleString()})</option>`; });
  document.getElementById('lotlist-no-data').style.display='none'; document.getElementById('lotlist-table').style.display='table';
  ...
  llFilteredRows=[...llAllRows]; llPage=1; renderLotListTable();
}

function renderLotListTable() {
  const ds=activeDatasetId?datasets[activeDatasetId]:null;
  const thead=document.getElementById('lotlist-thead');
  thead.innerHTML=`<tr><th style="width:40px;">#</th>${llColumns.map(col => `<th onclick="lotListSort('${col}')" class="${llSortCol===col?'sorted':''}">${col}<span class="sort-arrow">${llSortCol===col?(llSortDir===1?'▲':'▼'):'⇅'}</span></th>`).join('')}<th style="width:80px;">Action</th></tr>`;
  const total=llFilteredRows.length, totalPages=Math.max(1,Math.ceil(total/LL_PAGE_SIZE));
  llPage=Math.min(llPage,totalPages);
  const start=(llPage-1)*LL_PAGE_SIZE, end=Math.min(start+LL_PAGE_SIZE,total);
  const pageRows=llFilteredRows.slice(start,end);
  const tbody=document.getElementById('lotlist-tbody');
  if (!pageRows.length) { tbody.innerHTML=`<tr><td colspan="${llColumns.length+2}" style="text-align:center;padding:36px;color:rgba(66,66,66,0.38);font-size:12px;">No lots match your search</td></tr>`; }
  else { tbody.innerHTML=pageRows.map((row,i) => { const cls=ds&&ds.classProp?(row[ds.classProp]||''):''; const color=cls?getColor(cls):'rgba(66,66,66,0.15)'; const cells=llColumns.map(col => { const val=row[col]??'—'; if (col===ds?.classProp) return `<td><span class="lot-class-badge" style="background:${color}22;color:${color};">${val}</span></td>`; return `<td title="${val}">${val}</td>`; }).join(''); return `<tr><td style="color:rgba(66,66,66,0.35);font-size:10px;">${start+i+1}</td>${cells}<td><button class="lot-action-btn" onclick="lotListZoomTo(${row._idx})">🔍 Zoom</button></td></tr>`; }).join(''); }
  ...
  let pageBtns=`<button class="ll-page-btn" onclick="llGoPage(${llPage-1})" ${llPage===1?'disabled':''}>‹</button>`;
  buildPageRange(llPage,Math.max(1,Math.ceil(total/LL_PAGE_SIZE)),5).forEach(p => { if (p==='…') pageBtns+=`<button class="ll-page-btn" disabled style="border:none;background:none;">…</button>`; else pageBtns+=`<button class="ll-page-btn ${p===llPage?'active':''}" onclick="llGoPage(${p})">${p}</button>`; });
...
function buildPageRange(current, total, delta) {
  const range=[]; for (let i=Math.max(1,current-delta);i<=Math.min(total,current+delta);i++) range.push(i);
...
function llGoPage(p) { const total=llFilteredRows.length, totalPages=Math.max(1,Math.ceil(total/LL_PAGE_SIZE)); llPage=Math.max(1,Math.min(p,totalPages)); renderLotListTable(); document.querySelector('.lotlist-table-wrap').scrollTop=0; }
function lotListSearch(val) { const q=val.toLowerCase().trim(), classFilter=document.getElementById('lotlist-class-filter').value, ds=activeDatasetId?datasets[activeDatasetId]:null; llFilteredRows=llAllRows.filter(row => { const classMatch=!classFilter||(ds&&row[ds.classProp]===classFilter); const textMatch=!q||llColumns.some(col => String(row[col]||'').toLowerCase().includes(q)); return classMatch&&textMatch; }); llPage=1; renderLotListTable(); }
function lotListFilterClass(cls) { const q=document.getElementById('lotlist-search-input').value.toLowerCase().trim(), ds=activeDatasetId?datasets[activeDatasetId]:null; llFilteredRows=llAllRows.filter(row => { const classMatch=!cls||(ds&&row[ds.classProp]===cls); const textMatch=!q||llColumns.some(col => String(row[col]||'').toLowerCase().includes(q)); return classMatch&&textMatch; }); llPage=1; renderLotListTable(); }
function lotListSort(col) { if (llSortCol===col) llSortDir*=-1; else { llSortCol=col; llSortDir=1; } llFilteredRows.sort((a,b) => { const av=a[col]??,bv=b[col]??''; if (!isNaN(av)&&!isNaN(bv)) return (parseFloat(av)-parseFloat(bv))*llSortDir; return String(av).localeCompare(String(bv))*llSortDir; }); llPage=1; renderLotListTable(); }
function lotListZoomTo(featureIdx) { if (!activeDatasetId) return; const ds=datasets[activeDatasetId], feature=ds.features[featureIdx]; if (!feature) return; const coords=flattenCoords(feature.geometry); if (!coords.length) return; const avgLng=coords.reduce((s,[lng]) => s+lng,0)/coords.length, avgLat=coords.reduce((s,[,lat]) => s+lat,0)/coords.length; map.flyTo({ center:[avgLng,avgLat], zoom:17 }); closeLotListModal(); showToast('🔍 Zoomed to lot'); }
function exportLotListCSV() { if (!activeDatasetId) { showToast('⚠️ No dataset loaded'); return; } const ds=datasets[activeDatasetId], rows=llFilteredRows.length>0?llFilteredRows:llAllRows; const header=llColumns.join(','), body=rows.map(row => llColumns.map(col => { const val=row[col]??''; return typeof val==='string'&&(val.includes(',')||val.includes('"'))?`"${val.replace(/"/g,'""')}"`:val; }).join(',')).join('\n'); const blob=new Blob([header+'\n'+body],{ type:'text/csv' }); triggerDownload(blob,ds.name.replace(/\.[^.]+$/,'')+'-lotlist.csv'); showToast('✅ CSV exported · '+rows.length.toLocaleString()+' rows'); addActivityLog('Lot list exported (CSV)',rows.length.toLocaleString()+' rows · '+ds.name); }

// ============================================================
// HISTORY ANALYSIS
// ============================================================
const historyLog=[{ title:'Dashboard initialised', detail:'Ready to receive GeoJSON data', time:new Date().toLocaleTimeString('en-MY'), category:'system', icon:'🚀' }];
...
function addActivityLog(title, sub) {
  ...
  else if (t.includes('export')||t.includes('pdf')||t.includes('png')) { category='export'; icon='📄'; }
...
function openHistoryModal() { renderHistoryTimeline(); document.getElementById('history-overlay').classList.add('show'); }
function closeHistoryModal() { document.getElementById('history-overlay').classList.remove('show'); }
function filterHistory(cat, el) { historyFilter=cat; document.querySelectorAll('.hfilt').forEach(b => b.classList.remove('active')); el.classList.add('active'); renderHistoryTimeline(); }
function clearHistoryLog() { historyLog.length=0; historyLog.push({ title:'Log cleared', detail:'History reset by user', time:new Date().toLocaleTimeString('en-MY'), category:'system', icon:'🗑️' }); document.getElementById('activity-list').innerHTML=''; renderHistoryTimeline(); }
...
function renderHistoryTimeline() {
  ...
  if (!filtered.length) { timeline.innerHTML='<div class="history-empty"><div style="font-size:28px;margin-bottom:8px;">📋</div><div style="font-size:12.5px;color:rgba(66,66,66,0.38);">No activity in this category</div></div>'; return; }
  ...
  timeline.innerHTML=filtered.map(e => `<div class="hist-item"><div class="hist-icon" style="background:${iconBg[e.category]||'rgba(66,66,66,0.08)'};">${e.icon}</div><div class="hist-body"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;"><span class="hist-title">${e.title}</span><span class="hist-tag ${tagClass[e.category]||'tag-system'}">${e.category}</span></div><div class="hist-detail">${e.detail}</div><div class="hist-time">${e.time}</div></div></div>`).join('');
}

function showToast(msg, dur=3000) { if (!msg) return; const t=document.getElementById('toast'); t.textContent=msg; t.style.display='block'; setTimeout(() => t.style.display='none',dur); }
function showLoading(text, sub) { document.getElementById('loading-text').textContent=text; document.getElementById('loading-sub').textContent=sub; document.getElementById('loading-overlay').classList.add('show'); }
